...
public class ClassLoaderInjector {
    ...
    private static ClassLoader createNewClassLoader(Context context, ClassLoader oldClassLoader, List<File> patchs) throws Throwable {
         ...
         */
        //得到原本的pathList
        Field pathListField = ReflectUtils.findField(oldClassLoader, "pathList");
        Object oldPathList = pathListField.get(oldClassLoader);

        //dexElements
        Field dexElementsField = ReflectUtils.findField(oldPathList, "dexElements");
        Object[] oldDexElements = (Object[]) dexElementsField.get(oldPathList);

        //从Element上得到 dexFile
        Field dexFileField = ReflectUtils.findField(oldDexElements[0], "dexFile");
        ...
        Field nativeLibraryDirectoriesField = ReflectUtils.findField(oldPathList, "nativeLibraryDirectories");
    ...
    private static void doInject(Application app, ClassLoader classLoader) throws Throwable {
        ...
        Context baseContext = (Context) ReflectUtils.findField(app, "mBase").get(app);
        if (Build.VERSION.SDK_INT >= 26) {
            ReflectUtils.findField(baseContext, "mClassLoader").set(baseContext, classLoader);
        }

        Object basePackageInfo = ReflectUtils.findField(baseContext, "mPackageInfo").get(baseContext);
        ReflectUtils.findField(basePackageInfo, "mClassLoader").set(basePackageInfo, classLoader);

        if (Build.VERSION.SDK_INT < 27) {
            Resources res = app.getResources();
            try {
                ReflectUtils.findField(res, "mClassLoader").set(res, classLoader);

                final Object drawableInflater = ReflectUtils.findField(res, "mDrawableInflater").get(res);
                if (drawableInflater != null) {
                    ReflectUtils.findField(drawableInflater, "mClassLoader").set(drawableInflater, classLoader);
                }
